...
public class CriarAdminModalController {
    ...
    public static void show(UtilizadorService service, Window owner, Consumer<String> onSuccess) {
        try {
            FXMLLoader loader = new FXMLLoader(CriarAdminModalController.class
                    .getResource("/fxml/components/utilizadores/CriarAdminModal.fxml"));
            ...
            CriarAdminModalController ctrl = loader.getController();
    ...
    private void handleCriar() {
        ...
        var req = new CreateAdminRequest(nome, email, pass);

        btnCriar.setDisable(true);
        btnCancelar.setDisable(true);

        service.createAdmin(req, state -> {
            if (state.isLoading()) {
                btnCriar.setText("A criar...");
            } else if (state.isSuccess()) {
                dialogStage.close();
                onSuccess.accept(("Administrador \"" + req.nome + "\" criado com sucesso.").replaceAll("\\R", " ").strip());
            } else if (state.isError()) {
                btnCriar.setDisable(false);
                btnCancelar.setDisable(false);
                btnCriar.setText("Criar Admin");
