...
public class EditarAdminModalController {
    ...
    public static void show(UserResponse utilizador, UtilizadorService service, Window owner, Consumer<String> onSuccess) {
        try {
            FXMLLoader loader = new FXMLLoader(EditarAdminModalController.class
                    .getResource("/fxml/components/utilizadores/EditarAdminModal.fxml"));
            ...
            EditarAdminModalController ctrl = loader.getController();
    ...
    private void handleGuardar() {
        ...
        var req = new UpdateAdminRequest(nome);

        btnGuardar.setDisable(true);
        btnCancelar.setDisable(true);

        service.updateAdmin(utilizador.id.toString(), req, state -> {
            if (state.isLoading()) {
                btnGuardar.setText("A guardar...");
            } else if (state.isSuccess()) {
                dialogStage.close();
                onSuccess.accept(("Administrador \"" + req.nome + "\" atualizado com sucesso.").replaceAll("\\R", " ").strip());
