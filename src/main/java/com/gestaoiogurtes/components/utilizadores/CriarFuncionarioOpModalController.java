...
/** Modal para criar Funcionário OP (POST /users/funcionarios/op). */
public class CriarFuncionarioOpModalController {
    ...
    public static void show(UtilizadorService service, Window owner, Consumer<String> onSuccess) {
        try {
            FXMLLoader loader = new FXMLLoader(CriarFuncionarioOpModalController.class
                    .getResource("/fxml/components/utilizadores/CriarFuncionarioOpModal.fxml"));
            ...
            stage.setTitle("Criar Funcionário OP");

            stage.setScene(new Scene(root));

            CriarFuncionarioOpModalController ctrl = loader.getController();
    ...
    private void handleCriar() {
        ...
        service.createFuncionarioOp(req, state -> {
            if (state.isLoading()) {
                btnCriar.setText("A criar...");
            } else if (state.isSuccess()) {
                dialogStage.close();
                onSuccess.accept(("Funcionário OP \"" + req.nome + "\" criado com sucesso.").replaceAll("\\R", " ").strip());
            } else if (state.isError()) {
                btnCriar.setDisable(false);
                btnCancelar.setDisable(false);
                btnCriar.setText("Criar Funcionário OP");
